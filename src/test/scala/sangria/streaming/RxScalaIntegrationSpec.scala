...
class RxScalaIntegrationSpec extends AnyWordSpec with Matchers {
  ...
  "RxScala Integration" should {
    ...
    "mapFuture" in {
      res(
        impl.mapFuture(Observable.from(List(1, 2, 10)))(x =>
          Future.successful(x + 1))).toSet should be(Set(2, 3, 11))
    }
    ...
    "flatMapFuture" in {
      res(impl.flatMapFuture(Future.successful(1))(i =>
        Observable.from(List(i.toString, (i + 1).toString)))) should be(List("1", "2"))
    }
